<template>
  <div>
    ...
    <div v-if="isLoading" class="text-center py-20">
      <div class="w-10 h-10 border-3 border-gray-300 border-t-dark rounded-full animate-spin mx-auto mb-4"></div>
    ...
    <div v-else-if="error" class="text-center py-20">
      ...
      <RouterLink to="/admin/orders" class="btn btn-outline">返回订单列表</RouterLink>
    </div>

    <!-- Order detail -->
    <div v-else-if="order">
      <div class="flex items-center gap-4 mb-6">
        <RouterLink to="/admin/orders" class="text-gray-500 hover:text-dark transition-colors">&larr; 返回</RouterLink>
      ...
      <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="divide-y">
              <div v-for="item in order.items" :key="item.id" class="flex gap-4 py-3">
                <img
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p v-if="item.variantInfo" class="text-sm text-gray-500">{{ item.variantInfo }}</p>
                </div>
                <div class="text-right text-sm flex-shrink-0">
                  <p>${{ parseFloat(item.price).toFixed(2) }} x {{ item.quantity }}</p>
                  <p class="font-medium">${{ parseFloat(item.total).toFixed(2) }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- Address -->
          <div v-if="order.shippingAddress" class="bg-white rounded-lg shadow-sm p-6">
            <h2 class="font-semibold mb-3">收货地址</h2>
            <p>{{ order.shippingAddress.firstName }} {{ order.shippingAddress.lastName }}</p>
            <p class="text-gray-600 text-sm">{{ order.shippingAddress.street }}, {{ order.shippingAddress.city }}, {{ order.shippingAddress.state }} {{ order.shippingAddress.postalCode }}</p>
            <p class="text-gray-600 text-sm">{{ order.shippingAddress.phone }}</p>
          </div>
        </div>
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white rounded-lg shadow-sm p-6 space-y-3">
            ...
            <div v-if="parseFloat(order.discount) > 0" class="flex justify-between text-sm">
              <span class="text-gray-500">折扣</span>
              <span class="text-red-500">-${{ parseFloat(order.discount).toFixed(2) }}</span>
            </div>
            <hr />
            ...
            <div class="flex justify-between text-sm pt-2">
              ...
              <span class="inline-flex px-2 py-0.5 text-xs rounded-full" :class="paymentClass(order.paymentStatus)">
          ...
          <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div v-if="newStatus === 'SHIPPED'" class="mb-3">
              ...
              <input v-model="trackingNumber" type="text" class="input text-sm" placeholder="输入物流单号" />
            </div>
            <button
              @click="updateStatus"
...
const formatDate = (d) => new Date(d).toLocaleDateString('zh-CN', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
