<template>
  <div class="container-custom py-12">
    ...
    <nav class="mb-8 text-sm">
      ...
      <RouterLink to="/shop" class="text-gray-500 hover:text-dark">商品</RouterLink>
      <span class="mx-2 text-gray-400">/</span>
    ...
    <!-- 商品详情 -->
    <div v-else-if="product" class="lg:flex gap-12">
      <!-- 图片区 -->
      <div class="lg:w-1/2">
        <!-- 主图 -->
        <div class="aspect-square bg-gray-100 mb-4">
          <img 
            :src="selectedImage?.url || product.images?.[0]?.url || '/placeholder.jpg'"
            :alt="product.name"
        ...
        <div v-if="product.images?.length > 1" class="flex gap-2">
          <button 
            ...
            :class="selectedImage?.id === img.id ? 'border-dark' : 'border-transparent'"
          >
            <img 
              :src="img.url"
      ...
      <div class="lg:w-1/2 py-8 lg:py-0">
        ...
        <!-- 评分 -->
        <div v-if="product.reviewCount > 0" class="flex items-center gap-2 mb-4">
          <div class="flex text-accent-400">
            <StarIcon v-for="i in 5" :key="i" class="w-4 h-4" :class="i <= Math.round(product.avgRating) ? 'fill-current' : ''" />
          </div>
        ...
        <div v-if="sizes.length > 0" class="mb-6">
          ...
          <div class="flex flex-wrap gap-2">
            <button 
              ...
              :class="[
                selectedSize === size ? 'border-dark bg-dark text-white' : 'hover:border-dark',
        ...
        <div v-if="colors.length > 0" class="mb-8">
          ...
          <div class="flex gap-2">
            <button 
              ...
              :class="selectedColor === color ? 'border-dark' : 'border-transparent'"
    ...
    <!-- 商品详细描述 -->
    <section v-if="product?.description" class="mt-20">
      <h2 class="text-2xl font-serif font-bold mb-6">商品详情</h2>
      <div class="prose max-w-none" v-html="product.description"></div>
    </section>

    <!-- 相关商品 -->
    <section v-if="relatedProducts.length > 0" class="mt-20">
      <h2 class="text-2xl font-serif font-bold mb-8">你可能也喜欢</h2>
...
const sizes = computed(() => {
  ...
  const sizeSet = new Set(product.value.variants.map(v => v.size).filter(Boolean))
  return Array.from(sizeSet).sort((a, b) => parseFloat(a) - parseFloat(b))
})

const colors = computed(() => {
  if (!product.value?.variants) return []
  const colorSet = new Set(product.value.variants.map(v => v.color).filter(Boolean))
...
const isSizeAvailable = (size) => {
  const variant = product.value.variants?.find(
    v => v.size === size && (!selectedColor.value || v.color === selectedColor.value)
  )
...
const canAddToCart = computed(() => {
  ...
  if (product.value.variants?.length > 0) {
    return selectedVariant.value && selectedVariant.value.stock >= quantity.value
  }
  return true
})
